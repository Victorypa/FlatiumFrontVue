<template>
<div>

  <basicheader></basicheader>

  <div class="projects">

    <div class="container-fluid ">
      <div class="row">
        <sidebar></sidebar>

        <div class="col-md-10">

          <div class="row col-10 fixed-part shadow bg-white rounded pl-3 align-items-center">
            <div class="col-md-8 pl-1">
              <h1 class="main-caption">
                Акт выполненных работ №1
              </h1>
            </div>


            <div class="col-md-2 ml-auto">
              <button type="button" class="primary-button w-100">Редактировать</button>
            </div>

            <div class="col-md-6 pt-3 pl-1">
              <h2 class="main-subtitle px-15"> Итого: 2 120 000 Р</h2>
            </div>
             <div class="col-md-6 pt-3 d-flex justify-content-end px-0">
               <div class="col-4 text-center">
                  <h2 class="main-subtitle px-15"> 1 120 000 Р</h2>
               </div>
          
            </div>

          </div>

          <div class="col-12 px-0">

            <div class="projects__content px-15">
                <h2 class="main-subtitle main-subtitle--room">
                  Комната 1
                </h2>
                <div class="projects__desc col-8 d-flex justify-content-between align-items-center pt-3 px-0">

                  <div class="projects__desc-item">Общая площадь: 28 м<sup>2</sup></div>
                  <div class="projects__desc-item">Высота потолка: 2,8 м</div>
                  <div class="projects__desc-item">Площадь стен: 80,1 м<sup>2</sup></div>
                  <div class="projects__desc-item">Периметр: 29,8</div>

                </div>
              </div>

              <div class="projects__information ">

                <div class="row bg px-3">

                  <div class="main-subtitle main-subtitle--fz col-12 pt-4 pl-2">
                    Пол
                  </div>

                  <div class="col-12 px-0">

                    <table class="table table-hover">

                      <tbody>
                          <tr>
                          <th scope="row" class="w-50 pl-1">
                          <div class="form-check custom-control checkbox">
                          <input class="form-check-input check" id="1" type="checkbox">
                          <label class="form-check-label d-block" for="1">
                          Наименование
                          </label>
                          </div>
                          </th>
                          <td class="d-none">28м<sup>2</sup></td>
                          <td class="py-1 pl-1"><input class="col-4 form-control" type="text" placeholder="28м²"></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                                      <tr class="small-case">
                          <th scope="row" class="w-50">Наименование 1</th>
                          <td></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                                     <tr class="small-case">
                            <th scope="row" class="w-50">Наименование 1</th>
                          <td></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                                      <tr class="small-case">
                            <th scope="row" class="w-50">Наименование 1</th>
                          <td></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                            <tr>
                          <th scope="row" class="w-50 pl-1">
                          <div class="form-check custom-control checkbox">
                          <input class="form-check-input check" id="2" type="checkbox">
                          <label class="form-check-label d-block" for="2">
                          Наименование
                          </label>
                          </div>
                          </th>
                          <td>28м<sup>2</sup></td>
                          <!-- <td class="py-1 pl-1"><input class="col-4 form-control" type="text" placeholder="28м²"></td> -->
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                
                      </tbody>
                    </table>

                  </div>
                </div>

                <div class="row bg px-3">

                  <div class="main-subtitle main-subtitle--fz col-12 pl-2">
                    Стены
                  </div>

                  <div class="col-12  px-0">

                    <table class="table table-hover">

                      <tbody>
                        <tr>
                          <th scope="row" class="w-50">Очистка стен от старых обоев</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Демонтаж дверей</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Демонтаж старой штукатурки</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Грунтовка стен (3 цикла)</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Штукатурка стены по маякам</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Шпатлёвка в 2 слоя</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Шлифовка стен</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Окраска стен в 2 слоя</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Оклейка обоями под покраску</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Наименование работы</th>
                          <td>28 м<sup>2</sup></td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                      </tbody>
                    </table>

                  </div>
                </div>

                <div class="row bg px-3">

                  <div class="main-subtitle main-subtitle--fz col-12 pl-2">
                    Потолок
                  </div>

                  <div class="col-12  px-0">

                    <table class="table table-hover">

                      <tbody>
                        <tr>
                          <th scope="row" class="w-50">Наименование работы</th>
                          <td>28м2</td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Наименование работы</th>
                          <td>28м2</td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Наименование работы</th>
                          <td>28м2</td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Наименование работы</th>
                          <td>28м2</td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>
                        <tr>
                          <th scope="row">Наименование работы</th>
                          <td>28м2</td>
                          <td>100 Р/М<sup>2</sup></td>
                          <td>2800 Р</td>
                        </tr>

                      </tbody>
                    </table>

                  </div>
                </div>

              </div>

            </div>

          </div>
        </div>
      </div>
    </div>
  </div>
</template>


<script>
import basicheader from "./partials/BasicHeader";
import sidebar from "./partials/Sidebar";
export default {
  components: {
    basicheader,
    sidebar
  }
};
</script>

<style lang="scss" scoped>
@import "../assets/scss/vars.scss";
.fixed-part {
  position: fixed;

  background-color: $white;

  padding-bottom: 35px;
  padding-top: 85px;

  z-index: 999;
}

.projects {
  &__content {
    padding-top: 250px;
    padding-bottom: 20px;

    .form-group {
      margin-bottom: 0;

      input {
        &:first-child {
          margin-left: 0;
        }

        margin-left: 10px;
      }
    }

    .form-control {
      color: 666;
      height: 45px;
    }

    .custom-control {
      padding-left: 1.25rem;
    }
  }

  &__desc {
    font-weight: bold;
    color: $text-color;
  }

  &__information {
    .table {
      color: $text-color;

      td {
        width: 13%;
      }

      th {
        font-weight: normal;
      }
    }
  }
}

.form-check {
  &-label {
    padding-top: 1px;
    @include transition;
    cursor: pointer;

    &:before {
      border: 1px solid $ccc;
      border-radius: 0;
    }

    &::after {
      position: absolute;
      left: -18px;
      top: 5px;
      padding-left: 3px;
      font-size: 11px;
      color: $main-color;
    }

    &:hover {
      color: $button-hover;
    }
  }

  input[type="checkbox"]:checked + label::after,
  .abc-checkbox input[type="radio"]:checked + label::after {
    font-family: "FontAwesome";
    content: "\f00c";
  }

  label {
    cursor: pointer;
    display: inline;
    vertical-align: top;
    position: relative;
    padding-left: 5px;

    &::before {
      content: "";
      position: absolute;
      width: 20px;
      height: 20px;
      top: 3px;
      left: 0px;

      margin-left: -1.25rem;
      border-radius: 0;
      background-color: #fff;
      transition: border 0.15s ease-in-out, color 0.15s ease-in-out;
    }
  }
}

.small-case {
  font-size: 0.8rem;
  th {
    &:first-child {
      padding-left: 60px;
    }
  }
}

tr:hover {
  cursor: pointer;
}
</style>

